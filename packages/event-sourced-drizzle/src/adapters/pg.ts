/**
 * Reference Postgres/PGlite adapter for `createEventSourcedDrizzle`.
 *
 * Same interface as the SQLite adapter but uses Drizzle's PG query builder.
 */
...
/**
 * Creates a DrizzleAdapter backed by a Drizzle Postgres database.
 */
export function createPgAdapter(db: PgDb, config: PgAdapterConfig): DrizzleAdapter {
